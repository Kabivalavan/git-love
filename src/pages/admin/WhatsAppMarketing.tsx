...
import { MessageSquare, Send, Save, Loader2, Plus, Edit, Eye, ShoppingCart, Truck, Star, UserPlus, Package, Clock, Sparkles } from 'lucide-react';
...
const DEFAULT_TEMPLATES: WhatsAppTemplate[] = [
  ...
  },
  {
    ...
    description: 'Sent after delivery to request a review',
    is_active: true,
...
export default function WhatsAppMarketing() {
  ...
  const fetchTemplates = async () => {
    ...
    if (data?.value) {
      ...
      // Merge with defaults (add any new defaults not in saved)
      const savedIds = new Set(saved.map(t => t.id));
      const merged = [
        ...saved,
        ...DEFAULT_TEMPLATES.filter(d => !savedIds.has(d.id)),
      ];
      setTemplates(merged);
    }
    setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    const updated = templates.map(t =>
      t.id === editingTemplate.id
        ? { ...t, message: editMessage, is_active: editActive }
        : t
    );
  ...
  const getPreviewMessage = (template: WhatsAppTemplate) => {
    return template.message
      ...
      .replace(/\{\{coupon_code\}\}/g, 'WELCOME10')
      .replace(/\{\{discount\}\}/g, '10')
      .replace(/\{\{shop_url\}\}/g, 'https://mystore.com')
      .replace(/\{\{cart_items\}\}/g, 'Blue T-Shirt x1, Sneakers x1')
      .replace(/\{\{cart_total\}\}/g, '1,999')
      .replace(/\{\{checkout_url\}\}/g, 'https://mystore.com/checkout')
      .replace(/\{\{order_number\}\}/g, 'ORD20250320001')
      .replace(/\{\{order_items\}\}/g, 'Blue T-Shirt x1')
      .replace(/\{\{order_total\}\}/g, '999')
      .replace(/\{\{delivery_address\}\}/g, '123 Main St, Mumbai')
      .replace(/\{\{courier_name\}\}/g, 'Delhivery')
      .replace(/\{\{tracking_number\}\}/g, 'DL123456789')
      .replace(/\{\{estimated_delivery\}\}/g, '22 Mar 2025')
      .replace(/\{\{tracking_url\}\}/g, 'https://track.delhivery.com/123')
      .replace(/\{\{cod_message\}\}/g, 'COD Amount: ₹999')
      .replace(/\{\{review_url\}\}/g, 'https://mystore.com/review')
      .replace(/\{\{next_order_coupon\}\}/g, 'COMEBACK10')
      .replace(/\{\{product_name\}\}/g, 'Blue T-Shirt')
      .replace(/\{\{amount\}\}/g, '999');
  };

  return (
    <AdminLayout title="WhatsApp Marketing" description="Manage WhatsApp message templates for customer engagement">
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-green-600">{templates.filter(t => t.is_active).length}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{new Set(templates.map(t => t.trigger)).size}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-muted-foreground">{templates.filter(t => !t.is_active).length}</p>
              </div>
            </CardContent>
          </Card>
        </div>
                ...
                <div className="mt-2 bg-[#e5ddd5] rounded-xl p-4">
                  <div className="bg-white rounded-lg p-3 max-w-[85%] shadow-sm">
                    <p className="text-sm whitespace-pre-wrap leading-relaxed">
                      {getPreviewMessage({ ...editingTemplate, message: editMessage })}
                    </p>
