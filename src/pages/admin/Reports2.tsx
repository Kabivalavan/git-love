...
import {
  ...
  PieChart, Pie, Cell, LineChart, Line, AreaChart, Area, Legend, Funnel, FunnelChart,
} from 'recharts';
import {
  TrendingUp, TrendingDown, ShoppingCart, Users, DollarSign, Package,
  ArrowUpRight, ArrowDownRight, Percent, CreditCard, BarChart3, Truck,
...
function KPICard({ title, value, change, icon: Icon, color = 'primary' }: {
    ...
    <Card className="relative overflow-hidden">
      <CardContent className="p-4">
        <div className="flex items-start justify-between">
          ...
          <div className={`h-9 w-9 rounded-lg bg-${color}/10 flex items-center justify-center flex-shrink-0`}
...
export default function Reports2() {
  ...
  const { data: orders = [], isLoading } = useQuery({
    queryKey: ['r2-orders', fromStr, toStr],
    queryFn: async () => {
  ...
  const { data: prevOrders = [] } = useQuery({
    queryKey: ['r2-prev-orders', prevFromStr, prevToStr],
    queryFn: async () => {
      const { data } = await supabase.from('orders')
  ...
  const { data: orderItems = [] } = useQuery({
    queryKey: ['r2-items', fromStr, toStr],
    queryFn: async () => {
  ...
  const { data: expenses = [] } = useQuery({
    queryKey: ['r2-expenses', fromStr, toStr],
    queryFn: async () => {
      const { data } = await supabase.from('expenses')
        .select('amount, category, date')
        .gte('date', format(from, 'yyyy-MM-dd')).lte('date', format(to, 'yyyy-MM-dd'));
      return data || [];
    },
  });
  ...
  const { data: customers = [] } = useQuery({
    queryKey: ['r2-customers', fromStr, toStr],
    queryFn: async () => {
  ...
  const { data: deliveries = [] } = useQuery({
    queryKey: ['r2-deliveries', fromStr, toStr],
    queryFn: async () => {
  ...
  const { data: payments = [] } = useQuery({
    queryKey: ['r2-payments', fromStr, toStr],
    queryFn: async () => {
  ...
  const { data: refunds = [] } = useQuery({
    queryKey: ['r2-refunds', fromStr, toStr],
    queryFn: async () => {
  ...
  const analytics = useMemo(() => {
    ...
    const paidOrders = fo.filter(o => o.payment_status === 'paid');
    ...
    // Payments yet to receive (COD pending)
    const codPendingAmount = fo
    ...
    // Subtract daily expenses proportionally
    ...
    const topCustomers = topCustomerIds.map(([uid, data]) => {
      const profile = customers.find(c => c.user_id === uid);
    ...
    expenses.forEach(e => { expCatMap[e.category] = (expCatMap[e.category] || 0) + Number(e.amount || 0); });
    ...
    deliveries.forEach(d => { delStatusMap[d.status || 'pending'] = (delStatusMap[d.status || 'pending'] || 0) + 1; });

    // Cashflow
    const paymentsReceived = payments.filter(p => p.status === 'paid').reduce((s, p) => s + Number(p.amount || 0), 0);
    const refundTotal = refunds.reduce((s, r) => s + Number(r.amount || 0), 0);

    // AI Insights
    const insights: string[] = [];
    if (codPct > 60) insights.push(`⚠️ COD orders at ${codPct.toFixed(0)}% — consider incentivizing online payments`);
    if (topProducts[0]) insights.push(`🔥 "${topProducts[0].name}" generating ₹${Math.round(topProducts[0].revenue).toLocaleString()} revenue`);
    if (profit < 0) insights.push(`📉 Net profit is negative — review expenses (₹${Math.round(totalExpenses).toLocaleString()})`);
    ...
    if (revenueChange < -10) insights.push(`📉 Revenue down ${Math.abs(revenueChange).toFixed(0)}% — investigate drop in traffic or conversions`);
    if (totalDiscount > totalRevenue * 0.15) insights.push(`🎯 Discounts are ${((totalDiscount / totalRevenue) * 100).toFixed(0)}% of revenue — review discount strategy`);
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2.5 max-h-[250px] overflow-y-auto">
                  ...
                  {analytics.topProducts.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No data</p>}
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2.5 max-h-[220px] overflow-y-auto">
                  ...
                  {analytics.topCustomers.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No data</p>}
                </div>
              </CardContent>
            </Card>
