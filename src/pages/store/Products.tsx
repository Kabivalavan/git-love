    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <Input type="number" value={priceRange[0]} onChange={(e) => setPriceRange([parseInt(e.target.value) || 0, priceRange[1]])} className="w-20 text-sm" />
          <span className="text-muted-foreground">to</span>
          <Input type="number" value={priceRange[1]} onChange={(e) => setPriceRange([priceRange[0], parseInt(e.target.value) || 10000])} className="w-20 text-sm" />
        </div>
      </div>
      <div>
            ...
            <Sheet>
              <SheetTrigger asChild className="lg:hidden">
                <Button variant="outline" className="relative">
                  ...
                  Filters
                  {activeFilterCount > 0 && (
                    <Badge className="absolute -top-2 -right-2 h-5 w-5 p-0 flex items-center justify-center">{activeFilterCount}</Badge>
                  )}
                </Button>
              </SheetTrigger>
        ...
        {categorySlug && subCategories.length > 0 && (
          <div className="grid grid-cols-4 gap-3 sm:gap-4 mb-6">
            {subCategories.map((sub) => {
              ...
              return (
                <button key={sub.id} onClick={() => setSelectedSubCategory(isSelected ? null : sub.id)} className="group text-center">
