        ...
        >
          {products.slice(0, 8).map((product) => (
            <motion.div key={product.id} variants={scaleIn}>
              <ProductCard product={product} onAddToCart={handleAddToCart} onAddToWishlist={handleAddToWishlist} productOffer={getProductOffer(product)} lowStockSettings={storefrontDisplay} avgRating={reviewStats[product.id]?.avgRating || 0} reviewCount={reviewStats[product.id]?.reviewCount || 0} />
            </motion.div>
          ))}
        </motion.div>
      </div>
