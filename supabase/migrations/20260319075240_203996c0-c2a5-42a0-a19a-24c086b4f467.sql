...
-- Add show_on_cart column to coupons for cart page display toggle
ALTER TABLE public.coupons ADD COLUMN IF NOT EXISTS show_on_cart boolean DEFAULT false;

-- Auto-deactivate coupons that have reached usage limit or expired
UPDATE public.coupons
SET is_active = false
WHERE is_active = true
  AND (
    (usage_limit IS NOT NULL AND used_count >= usage_limit)
    OR (end_date IS NOT NULL AND end_date < now())
  );

-- Update get_homepage_data to include banner device visibility fields (already included via jsonb_agg)
-- and filter offers properly with auto_apply
  ...
  SELECT jsonb_build_object(
    ...
    'review_stats', COALESCE((
      WITH target_products AS (
        ...
        UNION
        SELECT latest.id FROM (SELECT id FROM products WHERE is_active = true ORDER BY created_at DESC LIMIT 8) AS latest
      )
      ...
      FROM (
        ...
        FROM reviews rv
        INNER JOIN target_products tp ON tp.id = rv.product_id
